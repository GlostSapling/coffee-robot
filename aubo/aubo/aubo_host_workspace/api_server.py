...
let twin = {
  ...
  joints: [0,0,0,0,0,0], ready: false, THREE: null,
  linkMeshes: [], linkPivots: [], linkAxes: [], linkSigns: []
};
async function initTwin(){
  ...
  try{
    ...
    if (box) box.textContent = '状态: 3D 资源加载失败';
    console.error(e);
    return;
  }
  ...
  const linkAxes = ["z","x","x","z","x","z"];
  const linkSigns = [-1, 1, 1, -1, 1, -1];

  // 加载所有 link GLB，直接放入 modelRoot
  const linkMeshes = [];
  ...
  for(let i = 0; i < 6; i++){
    (function(idx){
      loader.load('/api/twin/links/' + idx + '.glb?t=' + Date.now(), (gltf) => {
        const m = gltf.scene;
        m.traverse((obj) => {
          if (obj.isMesh) {
            ...
            obj.material = new THREE.MeshStandardMaterial({
              color: 0x94a3b8, metalness: 0.2, roughness: 0.72
            });
          }
        });
        modelRoot.add(m);
      ...
      }, undefined, (err) => { console.error('link'+idx+' load error:', err); });
    })(i);
  }
...
// 绕 CAD 坐标轴构建旋转矩阵
function _cadAxisAngleMat(axis, angle, THREE){
...
function updateTwinFromJoints(j){
  ...
  // 累积旋转：每个 link 受其自身及所有父关节旋转影响
  // R_cum[i] = R[0] * R[1] * ... * R[i]
  // 对每个 link 的每个顶点应用：
  //   new_pos = P[i] + R_cum[i-1] * (R[i] * (pos - P[i]) + P[i] - P[i-1] ... )
  // 简化：用关节链式变换
  //
  // 对于 link i：
  //   1. 平移到 J[i] 为原点
  //   2. 应用 J[i]..J[0] 的累积旋转（从 i 到 0 的顺序）
  //   3. 平移回原位

  // 构建累积旋转矩阵（从 J0 到 Ji）
  const cumR = [];
  let acc = new THREE.Matrix4(); // 单位矩阵
  for (let i = 0; i < 6; i++) {
    const angle = v[i] * twin.linkSigns[i];
    const Ri = _cadAxisAngleMat(twin.linkAxes[i], angle, THREE);
    acc = acc.clone().multiply(Ri);
    cumR.push(acc.clone());
  }

  // 对每个 link 应用变换
  // 算法：对于 link i，它绕 J[i] 旋转，并且跟随 J[0]..J[i-1] 的旋转
  // 世界坐标变换：pos' = P_chain + R_chain * (pos - P_chain)
  // 其中 P_chain 和 R_chain 是从 J0 到 Ji 的链式结果
  //
  // 分步：
  //   对于 link i：
  //     先计算 J[0..i] 的链式平移和旋转
  //     T_chain = J[0] + R[0]*(J[1] + R[1]*(...J[i]...))
  //     R_chain = R[0]*R[1]*...*R[i]
  //     pos' = T_chain + R_chain * (pos - J[i])  ... 不对
  //
  // 正确的链式变换（以 2 关节为例）：
  //   pos' = J0 + R0 * (J1 - J0 + R1 * (pos - J1))
  //
  // 推广：
  //   pos' = J0 + R0*(J1-J0 + R1*(J2-J1 + R2*(... + Ri*(pos - Ji)...)))
  //
  // 对于 link i 的每个顶点 pos：
  //   let p = pos - J[i]
  //   for k = i downto 0:
  //     p = R[k] * p + (J[k+1] - J[k])   (if k < i)
  //     p = R[k] * p                        (if k == i)
  //   p += J[0]
  //
  // 但这需要逐顶点操作，太慢。用 Three.js 的 matrixAutoUpdate = false 直接设置矩阵。

  // 链式变换矩阵方法：
  // 对于 link i，其世界矩阵 = T(J0) * R(J0) * T(J1-J0) * R(J1) * ... * T(Ji-J{i-1}) * R(Ji)
  // 但 link 的几何体顶点是相对于世界原点的，所以还需要考虑 link 几何体的偏移

  // 简化方案：对每个 link，构建从世界原点到该 link 的完整变换链
  // M_i = T(P0) * R0 * T(P1-P0) * R1 * ... * T(Pi-P{i-1}) * Ri * T(-Pi)
  // 最后 T(-Pi) 是把顶点从以 Pi 为原点转为以世界原点为原点

  // 但 Three.js 的 Object3D.matrix 是相对于父级的，不直接支持世界矩阵设置
  // 更好的方式：用模型容器的 matrixAutoUpdate = false 直接设置世界变换

  for (let i = 0; i < 6; i++) {
    const m = twin.linkMeshes[i];
    if (!m) continue;

    // 链式变换（CAD 坐标系，mm）：
    // M_cad = T(P0) * R0 * T(P1-P0) * R1 * ... * T(Pi-P{i-1}) * Ri * T(-Pi)
    // 最终世界矩阵 = Scale(0.001) * RotX(-90°) * M_cad
    let Mcad = new THREE.Matrix4();
    Mcad.makeTranslation(twin.linkPivots[0][0], twin.linkPivots[0][1], twin.linkPivots[0][2]);

    for (let k = 0; k <= i; k++) {
      const angle = v[k] * twin.linkSigns[k];
      Mcad.multiply(_cadAxisAngleMat(twin.linkAxes[k], angle, THREE));
      if (k < i) {
        const p = twin.linkPivots[k], q = twin.linkPivots[k+1];
        Mcad.multiply(new THREE.Matrix4().makeTranslation(q[0]-p[0], q[1]-p[1], q[2]-p[2]));
      }
    }
    const pi = twin.linkPivots[i];
    Mcad.multiply(new THREE.Matrix4().makeTranslation(-pi[0], -pi[1], -pi[2]));

    // CAD -> Three.js: Scale(0.001) * RotX(-90°) * Mcad
    const cadToThree = new THREE.Matrix4()
      .makeScale(0.001, 0.001, 0.001)
      .multiply(new THREE.Matrix4().makeRotationX(-Math.PI / 2))
      .multiply(Mcad);

    m.matrixAutoUpdate = false;
    m.matrix.copy(cadToThree);
